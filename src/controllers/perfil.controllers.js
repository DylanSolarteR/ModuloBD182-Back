...
export const getPerfiles = async (req, res) => {
    try {
        ...
        if(perfiles.rows.length === 0) {
            return res.status(200).json({ message: 'La disciplina no tiene Perfiles' })
        }
...
export const getPerfilDisciplina = async (req, res) => {
    try {
        ...
        if(perfilDisciplina.rows.length === 0) {
            return res.status(200).json({ message: 'Descripciones de Disciplina y Perfil no encontrados' })
        }
